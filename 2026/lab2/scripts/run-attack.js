...
async function main() {
  ...
  const gasPrice = attackTx.gasPrice || attackReceipt.effectiveGasPrice;
  const gasFeeWei = attackReceipt.gasUsed.mul(gasPrice);
  ...
  console.log(
    "  Sau cauta tranzactia in Ganache GUI -> TRANSACTIONS -> click pe hash"
  );
  console.log(
  ...
  const attackData = {
    ...
    attackGasFeeEth: fmt6(gasFeeWei)
  };

  fs.writeFileSync(attackDataPath, `${JSON.stringify(attackData, null, 2)}\n`);
  console.log(
    "\nDate salvate in:",
    path.relative(process.cwd(), attackDataPath)
  );
}
